...
def unauthorized_add_dish_to_card_test(anon_client):
    request_data = {"dish_id": 1, "card_id": 1}
    response = anon_client.post(f"/{BASE_API_URL}/manage/add", data=request_data, content_type="application/json")
...
def unauthorized_remove_dish_from_card_test(anon_client):
    request_data = {"dish_id": 1, "card_id": 1}
    response = anon_client.post(f"/{BASE_API_URL}/manage/remove", data=request_data, content_type="application/json")
    assert response.status_code == HTTPStatus.UNAUTHORIZED
